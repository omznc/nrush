...
use crate::packages::{get_current_package_version, package_ranking, package_type, set_new_package_version};
...
async fn main() {
	...
	let fetch_version_results = futures::future::join_all(fetch_version_tasks.into_iter().map(|task| {
	...
	for result in fetch_version_results {
		match result {
			Ok((package, version)) => {
				...
				let current_version = get_current_package_version(&package, &json_data, &dev_package_names, &peer_package_names);
	...
	let generate_items = |to_update: &Vec<(String, String, bool, bool)>, json_data: &Value| -> Vec<String> {
		let mut items = vec![];
	...
	if !args.interactive && !args.update {
		...
		let user_input =
			prompt_confirm("\nDo you want to update all of these packages? (y/N)", false);
