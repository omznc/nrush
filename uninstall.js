...
const binp = path.join(cargoDir, "bin", "nrush");

if (fs.existsSync(binp)) {
  console.log("Uninstalling nrush...");
  exec(`cargo uninstall nrush`, (error, stdout, stderr) => {
    console.log(stdout);
    if (error || stderr) {
  ...
  console.log("nrush not found skipping!");
}
